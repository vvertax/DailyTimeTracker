...
- **Streak Shields + Keep Streak:** До `8` пропущенных дней в календарном месяце могут автоматически защищать серию, а опция `Keep streak` сохраняет текущую серию без роста.
...
- **Основные секции:** Compact mode показывает сокращенный Today view, а Full mode открывает полный список сессий, полную историю и недельную сводку.
- **Панель Settings:** Язык, режим popup, порог паузы, щиты серии, `Keep streak`, срок хранения истории, длинная прогрессия серии, экспорт, импорт, действия сброса и текст версии.
